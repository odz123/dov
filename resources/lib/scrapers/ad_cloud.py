		...
		try:
			...
			for item in self.scrape_results:
				try:
					...
					file_dl, size = item['link_dl'] if direct_debrid_link else item['link'], round(float(item['size'])/1073741824, 2)
	...
	def _scrape_folders(self, folder_id, filename):
		try:
			...
			for item in folder:
				try:
					item.update({
						'filename': item['n'], 'folder_name': filename, 'size': item['s'], 'link': item['l']
					})
					results_append(item)
				except Exception: pass
		except Exception: pass
