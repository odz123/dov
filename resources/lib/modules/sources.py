...
class SourceSelect:
	...
	def activate_external_providers(self):
		...
		# Check if Stremio is configured with addons (can provide streams - debrid may be server-side configured)
		self.stremio_configured = stremio_is_configured()
		if not self.debrid_torrent_enabled and not self.debrid_hoster_enabled and not self.stremio_configured:
			self._kill_progress_dialog()
