...
def people_get_imdb_id(actor_name, actor_tmdbID=None):
	def _get(dummy):
		...
		if actor_tmdbID:
			...
			except Exception as e: logger('imdb_api.people_get_imdb_id TMDB lookup', str(e))
		if not imdb_id:
	...
	if action in ('imdb_watchlist', 'imdb_user_list_contents', 'imdb_keywords_list_contents'):
		def _process():
			for item in items:
				...
				except Exception as e: logger('imdb_api._process list item', str(e))
		...
		def _process():
			for listing in all_reviews:
				...
				try:
					...
					except (IndexError, TypeError): rating = ''
					try:
		...
		except (IndexError, TypeError, ValueError): pages = [1]
